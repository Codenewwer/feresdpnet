...
pretrained_model_path = '/public/home/firststepbest_model.pth'
if os.path.exists(pretrained_model_path):
    print(f"Loading pretrained model from: {pretrained_model_path}")
    # 注意：如果 ESDPNet 结构与 MiniXception 差异巨大，这里的 load_state_dict 可能会报错或需要 strict=False
    try:
...
for name, param in model.named_parameters():
    # 这里可能需要根据 ESDPNet 的实际层名进行调整，暂时保留原样
    if any(seg in name for seg in ['conv1', 'bn1', 'layer0', 'layer1', 'layer2']):
...
# --- 最终评估：只评估测试集，并使用 classifier_evaluate ---
...
# 注意：classifier_evaluate 不需要概率值，只需要标签和预测类别
with torch.no_grad():
    for imgs, hogs, labels in test_loader:
        imgs = imgs.to(device, non_blocking=True)
        hogs = hogs.to(device, non_blocking=True)
